...
<head>
    ...
    <?php
        ...
        $sql = "SELECT a.TeamID, a.TeamName FROM team a
                INNER JOIN existinguser b
                WHERE b.CompanyID = ".$companyID."
                AND a.ManagerID = ".$userID."
                AND a.EndDate >= CURRENT_DATE()
                GROUP BY a.TeamID, a.TeamName;";
        ...
        if (isset($_POST['createAccount'])) {
            ...
            if ($result->num_rows > 0) {
                ...
                $stmt = $conn->prepare("INSERT INTO existinguser (CompanyID,SpecialisationID,TeamID,Role,FirstName,LastName,Gender,Email,Password,Status) VALUES (?,?,?,?,?,?,?,?,?,?)");

                $stmt->bind_param("iiissssssi",$companyID,$specialisationID,$teamID,$role,$firstName,$lastName,$gender,$email,$password,$userStatus);
