...
$sql_attendance = "SELECT ClockIn FROM attendance WHERE UserID = ? AND DATE(ClockIn) = ?";
...
if ($attendance && is_null($attendance['ClockOut'])) {
    ...
    $sql_update = "UPDATE attendance SET ClockOut = ?, TotalHours = TIMESTAMPDIFF(SECOND, ClockIn, ?) / 3600 WHERE UserID = ? AND DATE(ClockIn) = ?";
